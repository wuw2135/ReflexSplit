...
class ReflexModelBase(BaseModel):
...
class ReflexModel(ReflexModelBase):
    def name(self):
        return 'reflex'
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.scheduler_G = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer_G, T_max=10, eta_min=8e-6)
    ...
    def get_loss(self, out_t, out_r):
        ...
        loss_r_color = self.loss_dic['color_consistency'](out_r, self.target_r) * self.opt.lambda_color
    
        return loss_t_pixel, loss_r_pixel, loss_t_vgg, loss_exclu, loss_recons, loss_r_color


    def backward_G(self):
        (self.loss_t_pixel, self.loss_r_pixel,
        self.loss_t_vgg, self.loss_exclu, self.loss_recons, self.loss_r_color) = self.get_loss(self.output_t, self.output_r)
        
        self.loss_G = (self.loss_t_pixel + self.loss_r_pixel + self.loss_t_vgg + 
                    self.loss_exclu + self.loss_recons + self.loss_r_color)
    ...
    def forward(self):
        ...
        output_t, output_r, output_rr = self.network((input_i, fn))
    ...
    def get_current_errors(self):
        ...
        if self.loss_r_color is not None:
            ret_errors['R_Color'] = self.loss_r_color.item()
    ...
    def load_weights(self):
        ...
        if self.isTrain and 'scheduler' in state_dict.keys():
            self.scheduler_G.load_state_dict(state_dict['scheduler'])
        ...
        if self.opt.resume:
            ...
            if self.isTrain and 'scheduler' in state_dict.keys():
                try:
                    ...
                    print('Warning: scheduler type changed, using new scheduler')
            
            print('Resumed training from epoch %d, iteration %d' % (self.epoch, self.iterations))
        else:
            self.epoch = 0
            self.iterations = 0
            print('Loaded pretrained weights from epoch %d, iteration %d' % (state_dict['epoch'], state_dict['iterations']))
            print('Starting new training from epoch 0 with lr=%f' % self.opt.lr)
    ...
    def state_dict(self, save_extra_state=True):
        ...
        if save_extra_state and hasattr(self, 'optimizer_G'):
            state_dict.update({
