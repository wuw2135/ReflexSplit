...
class ColorConsistencyLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        loss_mean = self.criterion(pred_mean, target_mean)
        loss_std = self.criterion(pred_std, target_std)
        
        # 色彩直方圖相似性（可選）
        # 將圖像分成多個 bins 來比較色彩分佈
        pred_hist = torch.histc(pred, bins=64, min=0, max=1)
        target_hist = torch.histc(target, bins=64, min=0, max=1)
        loss_hist = self.criterion(pred_hist, target_hist)
        
        return loss_mean + loss_std + 0.1 * loss_hist
...
def init_loss(opt):
    ...
    loss_dic['t_vgg'] = VGGLoss()
    
    loss_dic['color_consistency'] = ColorConsistencyLoss()
